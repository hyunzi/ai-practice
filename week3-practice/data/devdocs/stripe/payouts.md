...
There are two valid bank account types: ?œchecking??and ?œsavings??
...
CÃ´te d?™Ivoire is only available for [Cross-border payouts](https://docs.stripe.com/connect/cross-border-payouts.md) accounts.
...
| Account owner name       | ?¤ãƒž?€?ãƒŠ??              |

Stripe supports futsÅ« (??€šé ?? or tÅza (å½“åº§?é‡‘) type bank accounts in Japan. For cross-border payouts, your bank must also participate in the Foreign Exchange Yen Clearing System (FXYCS).
...
TÃ¼rkiye is only available for [Cross-border payouts](https://docs.stripe.com/connect/cross-border-payouts.md) accounts.
...
These restrictions don?™t apply if you use [cross-border payouts](https://docs.stripe.com/connect/cross-border-payouts.md).
...
Use the following collapsed table to determine your country?™s payout speed. The initial payout speed applies to your first payout, and the default payout speed applies to subsequent payouts.
...
Use the following test bank and debit card numbers to trigger certain events when testing [payouts](https://docs.stripe.com/connect/payouts-connected-accounts.md). You can only use these values while testing with test secret keys.
