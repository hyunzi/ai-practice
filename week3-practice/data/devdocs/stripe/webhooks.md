...
post '/webhook' do
  ...
  if endpoint_secret
    ...
    begin
      ...
      puts "? ï¸  Webhook signature verification failed. #{e.message}"
      status 400
    end
  end
...
def my_webhook_view(request):
  ...
  if endpoint_secret:
        ...
        try:
            ...
            print('? ï¸  Webhook signature verification failed.' + str(e))
...
if ($endpoint_secret) {
  ...
  try {
    ...
    // Invalid signature
    echo '? ï¸  Webhook error while validating signature.';
    http_response_code(400);
    exit();
  }
}
 ...
 // I# If you defined your endpoint using the API or the Dashboard, check your webhook settings for your endpoint secret: https://dashboard.stripe.com/webhooks
  ...
  if(endpointSecret != null && sigHeader != null) {
      ...
      try {
          ...
          // Invalid signature
          System.out.println("? ï¸  Webhook error while validating signature.");
          response.status(400);
          return "";
      }
  }
...
app.post('/webhook', express.raw({type: 'application/json'}), (request, response) => {
  ...
  if (endpointSecret) {
    ...
    try {
      ...
      console.log(`? ï¸ Webhook signature verification failed.`, err.message);
