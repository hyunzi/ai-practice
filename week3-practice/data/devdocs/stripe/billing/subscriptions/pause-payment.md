...
Pausing payment collection is often used to temporarily offer your services for free. This is sometimes referred to as a ?œgrace period??if a customer needs additional time to pay or can?™t pay for one or more billing periods.
...
If you temporarily want to offer your services for free and you don?™t want to collect payment on the invoice (for example, a ?œgrace period??, you can void invoices that your subscription creates to make sure that your customers aren?™t charged and the subscription remains `status=active`. Use the Subscription ID to update `pause_collection[behavior]` to `void` and `pause_collection[resumes_at]` to the date you want to start collecting payments again.
