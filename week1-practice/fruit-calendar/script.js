const fruits = [
  ...
  { icon: "??", label: "체리", color: "#ef4444" },
  { icon: "??", label: "오렌지", color: "#f97316" },
  { icon: "??", label: "그린애플", color: "#a3e635" },
...
function selectFruit(fruit) {
  ...
  setHelper(`${fruit.label} 아이콘을 선택했습니다. 달력 날짜를 눌러 스탬프를 남겨보세요.`);
}
...
function createCell(dateObj) {
  ...
  cell.addEventListener("click", () => {
    ...
    setHelper(`${key} 일정에 ${state.selectedFruit.label}을(를) 기록했습니다.`);
  });
